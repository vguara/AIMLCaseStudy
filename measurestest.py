...
def measures(y_true, y_pred, y_prob, old_ds):
    """
    ...
    if old_ds:
        performance_metrics["fraud_capture_rate_1_percent"] = calculate_fraud_capture_rate(y_true, y_prob, 0.01)
        performance_metrics["fraud_capture_rate_10_percent"] = calculate_fraud_capture_rate(y_true, y_prob, 0.1)
        performance_metrics["fraud_capture_rate_30_percent"] = calculate_fraud_capture_rate(y_true, y_prob, 0.3)
...
def train_and_evaluate_models(ds, models, metrics, X_test, y_test, old_ds=False):
    """
    ...
    for ratio, subset in ds.subsets.items():
        ...
        for model in models:
            ...
            results = measures(y_test, y_pred, y_prob, old_ds)
            print(f"model : {model}")
...
def train_model(model_name, X_train, y_train):
    ...
    """
    if model_name == 'LR':
        model = LogisticRegression(max_iter=10000, C=0.1, penalty='l2')  # Increased max_iter for convergence
    elif model_name == 'SVM':
